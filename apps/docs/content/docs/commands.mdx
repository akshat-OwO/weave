...
```sh
weave create <name> [--cpus <count>] [--memory <GiB>] [--ttl <duration>] [--template <name-or-path>] [--mount <directory>...]
```

The default TTL is 10 minutes. A template can only be supplied when creating a new VM, not when restarting an existing one. Pass one or more existing directories after `--mount` to expose them in the guest. Mounts are read-only by default; append `:w` to a directory to make it writable. With no `--mount` flag, the VM has no host mounts.
