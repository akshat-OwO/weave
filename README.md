...
Weave passes `--mount-none` when it creates a VM and when `weave create` reconfigures an existing stopped VM. This prevents Lima from mounting host directories into the guest. `weave start` leaves that configuration unchanged. VMs retain their own virtual disk when stopped and continue to have network access. Use `weave kill <name>` when the disk and its data are no longer needed.
